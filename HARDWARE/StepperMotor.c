...
void MotorParamsInit(){
	float StepAngle[5]={0,1.8,0.9,0.45,0.225};//这里多出的StepAngle[0] 只是为了设置方便， 实际没有任何作用
	StepperMotor.subdivide = 1;
	StepperMotor.StepAngle = StepAngle[StepperMotor.subdivide];//依据细分读书设置 电机步转角
	StepperMotor.DIR=clockwise; //顺时针
	...
	if(TMP_Flag_End ==3 && TMP_Flag_Head == 1){
		...
		if(len < 15){
			for(t=0;t<len;t++){
				...
				//USART_SendData(USART1,BigMotor.USART_DATA[t]);
			}
			TMP_STA = 0;
			TMP_Flag_End = 0;
		}
